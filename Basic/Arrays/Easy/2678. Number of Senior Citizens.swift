...
class Solution {
    func countSeniors(_ details: [String]) -> Int {
        var seniorCitizenCount = 0
        for detail in details {
            let detailArray = Array(detail)
            if let firstDigit = Int("\(detailArray[11])"), let secondDigit = Int("\(detailArray[12])") {
...
let result = Solution().countSeniors(details)
print(result)